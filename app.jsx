...
const buildPresentationDxf = (planSpec) => {
    ...
    const entities = [];
    const layerStyles = {
        OUTLINE: { name: 'OUTLINE', color: 8, lineweight: 30 },
        WALLS: { name: 'WALLS', color: 7, lineweight: 45 },
        DOORS: { name: 'DOORS', color: 1, lineweight: 25 },
        WINDOWS: { name: 'WINDOWS', color: 5, lineweight: 20 },
        LABELS: { name: 'LABELS', color: 2, lineweight: 18 },
        FURNITURE: { name: 'FURNITURE', color: 6, lineweight: 18 },
        DIMENSIONS: { name: 'DIMENSIONS', color: 4, lineweight: 15 },
        ELEVATION: { name: 'ELEVATION', color: 3, lineweight: 25 },
        ELEV_TEXT: { name: 'ELEV_TEXT', color: 2, lineweight: 15 },
        SHEET: { name: 'SHEET', color: 8, lineweight: 18 },
    };

    const push = (s) => entities.push(s);
    const finite = (...values) => values.every(v => Number.isFinite(v));
    const safeText = (value) => String(value || '').replace(/\n/g, ' ').replace(/[^\x20-\x7E]/g, '').trim();
    const styleForLayer = (layer) => layerStyles[layer] || { name: String(layer || '0'), color: 7, lineweight: 20 };
