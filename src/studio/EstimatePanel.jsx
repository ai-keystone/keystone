        ...
        <div className="paper-panel mt-4 overflow-hidden">
            <div className="p-4 md:p-5 border-b border-black/5 bg-white/40">
                <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-3">
                    ...
                    <span className="mono text-[8px] uppercase tracking-[0.22em]" style={{color:'var(--ink-soft)'}}>
            ...
            <div className="p-4 md:p-5">
                <div className="grid grid-cols-2 xl:grid-cols-5 gap-2 mb-4">
                    <div className="spec-panel" style={{gridColumn:'span 2'}}>
                        ...
                        <div className="mono text-[8px] uppercase mt-1" style={{color:'var(--ink-soft)'}}>Target {formatUsd(costRange?.total?.target)}</div>
                    </div>
                    ...
                    <div className="spec-panel"><div className="spec-label">Windows</div><div className="spec-value">{formatQuantity(summary.windowCount, 'total')}</div><div className="mono text-[8px] uppercase mt-1" style={{color:'var(--ink-soft)'}}>{formatQuantity(summary.roughGlazingAreaSqFt, 'sqft glazing')}</div></div>
                ...
                <div className="grid lg:grid-cols-2 gap-4">
                    <div className="rounded-[18px] border border-black/8 overflow-hidden bg-white/72">
                        <div className="px-4 py-3 border-b border-black/6">
                            <p className="mono text-[8px] uppercase tracking-[0.22em]" style={{color:'var(--ink-soft)'}}>Cost line items</p>
                        </div>
                        <div id={lineItemsId} className="overflow-x-auto">
                            <table className="w-full text-left">
                                <thead>
                                    <tr className="mono text-[8px] uppercase tracking-[0.18em]" style={{color:'var(--ink-soft)'}}>
                                ...
                                <tbody>
                                    {lineItemDisclosure.visible.map((item) => (
                                        <tr key={item.key} className="border-t border-black/5 text-[11px]" style={{color:'var(--ink)'}}>
                                            <td className="px-4 py-2.5">
                                                <div className="font-medium">{item.label}</div>
                                                {item.notes && <div className="text-[10px] mt-0.5" style={{color:'var(--ink-soft)'}}>{item.notes}</div>}
                                            </td>
                    ...
                    <div className="rounded-[18px] border border-black/8 overflow-hidden bg-white/72">
                        <div className="px-4 py-3 border-b border-black/6">
                            <p className="mono text-[8px] uppercase tracking-[0.22em]" style={{color:'var(--ink-soft)'}}>Quantity takeoff</p>
                        </div>
                ...
                <div id={assumptionsId} className="rounded-[18px] border border-black/8 bg-white/64 p-4 mt-4">
                    <p className="mono text-[8px] uppercase tracking-[0.22em]" style={{color:'var(--ink-soft)'}}>Assumptions</p>
                    ...
                    <div className="mt-3 text-[12px] leading-relaxed" style={{color:'var(--ink)'}}>
