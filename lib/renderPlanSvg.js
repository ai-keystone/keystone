  ...
  renderPlanSvg: function(planSpec) {
    ...
    planSpec.levels.forEach((lvl, index) => {
      ...
      const topDim = `
        ...
        <text x="${PAD + levelPixelW/2}" y="${startY - 35}" font-family="Arial" font-size="16" fill="#5B7C99" text-anchor="middle" font-weight="bold">${wFt}'</text>`;
      
      const sideDim = `
        <line x1="${PAD - 25}" y1="${startY}" x2="${PAD - 25}" y2="${startY + levelPixelH}" stroke="#5B7C99" stroke-width="2" />
        <text x="${PAD - 35}" y="${startY + levelPixelH/2}" font-family="Arial" font-size="16" fill="#5B7C99" text-anchor="end" dominant-baseline="middle" font-weight="bold">${hFt}'</text>`;
      ...
      const roomSvg = (lvl.rooms || []).map(r => {
        ...
        return `
          ...
          <text x="${rx + rw/2}" y="${ry + rh/2 - 8}" font-family="Arial" font-size="12" fill="#111" text-anchor="middle" font-weight="bold">${(r.label || r.type || "").toUpperCase()}</text>
          <text x="${rx + rw/2}" y="${ry + rh/2 + 10}" font-family="Arial" font-size="10" fill="#666" text-anchor="middle">${r.w}' x ${r.h}'</text>
        `;
      }).join("\n");
      ...
      const doorSvg = (lvl.doors || []).map(d => {
        ...
        return d.dir === "vertical" 
          ? `<rect x="${dx-4}" y="${dy-18}" width="8" height="36" fill="white" />`
          : `<rect x="${dx-18}" y="${dy-4}" width="36" height="8" fill="white" />`;
      }).join("\n");
      ...
      const header = `<text x="${PAD}" y="${yOffset + 40}" font-family="Arial" font-size="20" font-weight="bold" fill="#111">LEVEL ${levelNum} (${wFt * hFt} SQ FT)</text>`;
